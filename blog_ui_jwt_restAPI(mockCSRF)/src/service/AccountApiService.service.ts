...
constructor(
  ...
  public loginTest (): Promise<any> {
    ...
    return this._http.get(endpoint,{withCredentials: true}).pipe(
      map( (res:any) => {
        return res
      })
    )
  ...
  public register(userInformation: any) {
    ...
    return this._http.post(endpoint,userInformation, {
      withCredentials: true
    }).pipe(
      map( (res:any) => {
        return res
  ...
  public bindGitHub(bindID: any) {
    const endpoint = `http://localhost:8000/api/user/bindGitHub`
    return this._http.put(endpoint,bindID, {
      withCredentials: true
    }).pipe(
      map( (res:any) => {
        return res
